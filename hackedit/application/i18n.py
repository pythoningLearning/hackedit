...
def get_locale(settings):
    """
    Gets the current locale defined in the application settings
    """
    return settings.environment.locale


@inject(settings=Services.settings)
def set_locale(locale, settings=None):
    """
    Sets the application locale
    """
    settings.environment.locale = locale
...
def get_translation(package='hackedit'):
    ...
    try:
        t = gettext.translation(package, localedir=locale_dir, languages=[get_locale()])
    except OSError:
